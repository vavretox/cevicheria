<?php $__env->startSection('title', 'Gestion de Mesas'); ?>
    ...
    <div class="col-md-3">
        <div class="card">
            <div class="card-body">
                ...
                <div class="fs-3 fw-bold"><?php echo e($tables->where('ui_status', 'available')->count()); ?></div>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card">
            <div class="card-body">
                ...
                <div class="fs-3 fw-bold"><?php echo e($tables->where('ui_status', 'reserved')->count()); ?></div>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card">
            <div class="card-body">
                ...
                <div class="fs-3 fw-bold"><?php echo e($tables->where('ui_status', 'occupied')->count()); ?></div>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card">
            <div class="card-body">
                ...
                <div class="fs-3 fw-bold"><?php echo e($tables->where('ui_status', 'closed')->count()); ?></div>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        <div class="table-board">
            <?php $__empty_1 = true; $__currentLoopData = $tables; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $table): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="table-card <?php echo e($table->ui_status); ?>">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div class="table-card-title"><?php echo e($table->name); ?></div>
                        <span class="badge bg-light text-dark">
                            <?php if($table->ui_status === 'available'): ?>
                                Libre
                            <?php elseif($table->ui_status === 'reserved'): ?>
                                Reservada
                            <?php elseif($table->ui_status === 'occupied'): ?>
                                Ocupada
                            <?php else: ?>
                                Cerrada
                            <?php endif; ?>
                    ...
                    <div class="small text-muted mb-1"><?php echo e($table->zone ?: 'Salon principal'); ?></div>
                    <div class="small text-muted mb-2">Capacidad: <?php echo e($table->capacity ?: '-'); ?></div>
                    <?php if($table->isReserved()): ?>
                        <div class="small"><strong>Reserva:</strong> <?php echo e($table->reservation_name); ?></div>
                        <div class="small text-muted"><?php echo e($table->reservation_at?->format('d/m/Y H:i')); ?></div>
                    <?php endif; ?>
                    <?php if($table->reservation_notes): ?>
                        <div class="small text-muted mt-1"><?php echo e($table->reservation_notes); ?></div>
                    <?php endif; ?>
                    <div class="d-flex gap-2 mt-3">
                        <button
                            class="btn btn-sm btn-outline-dark"
                            onclick='showTableActivity(<?php echo e($table->id); ?>, <?php echo json_encode($table->name, 15, 512) ?>)'
                        >
                            <i class="fas fa-list"></i>
                        </button>
                        <button
                            class="btn btn-sm btn-outline-primary"
                            onclick='editTable(<?php echo e($table->id); ?>, <?php echo json_encode($table->name, 15, 512) ?>, <?php echo json_encode($table->zone, 15, 512) ?>, <?php echo e($table->capacity ? $table->capacity : 'null'); ?>, <?php echo e($table->active ? 'true' : 'false'); ?>)'
                        >
                            <i class="fas fa-pen"></i>
                        </button>
                        <button
                            class="btn btn-sm btn-outline-warning"
                            onclick='editReservation(<?php echo e($table->id); ?>, <?php echo json_encode($table->name, 15, 512) ?>, <?php echo json_encode($table->reservation_name, 15, 512) ?>, <?php echo json_encode(optional($table->reservation_at)->format('Y-m-d\\TH:i'), 15, 512) ?>, <?php echo json_encode($table->reservation_notes, 15, 512) ?>)'
                        >
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo e($table->zone ?: '-'); ?></td>
                        <td><?php echo e($table->capacity ?: '-'); ?></td>
                        <td>
                            <span class="badge <?php echo e($table->ui_status === 'available' ? 'bg-success' : ($table->ui_status === 'reserved' ? 'bg-warning text-dark' : ($table->ui_status === 'occupied' ? 'bg-primary' : 'bg-secondary'))); ?>">
                                <?php if($table->ui_status === 'available'): ?>
                                    Libre
                                <?php elseif($table->ui_status === 'reserved'): ?>
                                    Reservada
                        ...
                        <td>
                            <?php if($table->isReserved()): ?>
                                <div><?php echo e($table->reservation_name); ?></div>
                                <small class="text-muted"><?php echo e($table->reservation_at?->format('d/m/Y H:i')); ?></small>
                            <?php else: ?>
                        ...
                        <td class="text-end">
                            ...
                            <form action="<?php echo e(route('admin.tables.delete', $table->id)); ?>" method="POST" class="d-inline">
                                <?php echo csrf_field(); ?>
                                <?php echo method_field('DELETE'); ?>
                                <button type="submit" class="btn btn-sm btn-danger"
                                        onclick="return confirm('¿Eliminar esta mesa?')"
                                        <?php echo e($table->orders_count > 0 ? 'disabled' : ''); ?>>
...
function editTable(id, name, zone, capacity, active) {
    ...
    form.action = '<?php echo e(url("admin/tables")); ?>/' + id;
...
function editReservation(id, tableName, reservationName, reservationAt, reservationNotes) {
    ...
    form.action = '<?php echo e(url("admin/tables")); ?>/' + id + '/reservation';
...
function showTableActivity(id, tableName) {
    ...
    fetch('<?php echo e(url("admin/tables")); ?>/' + id + '/activity')
        .then(response => {
            if (!response.ok) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/cevicheria-pos/resources/views/admin/tables/index.blade.php ENDPATH**/ ?>